<template>
  <div
    v-if="isShow"
    :class="`mo-toasts inline-flex flex-col rounded overflow-hidden w-min border border-${theme} shadow border-opacity-40`"
  >
    ...
    <div
      v-if="$slots.detail"
      :class="`bg-light bg-opacity-80 p-2 text-sm text-${theme}`"
    >
      <slot name="detail" />
    </div>
    <div
      v-if="type !== 'message'"
      :class="`bg-light bg-opacity-80 p-2 box-border text-sm text-${theme} border-t border-${theme}`"
    >
      <slot name="operator">
        <div
